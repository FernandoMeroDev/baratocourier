...
class Landeables extends Component
{
    use WithPagination;
    ...
    {
        return view('livewire.shipments.land.landeables', [
    ...
    private function query()
    {
        ...
        $query = Shipment::join('users', 'shipments.user_id', '=', 'users.id')
            ...
            ->where(function($query) {
                $query->where('shipments.status', Shipment::$valid_statuses['shipment'])
                    ->orWhere('shipments.status', Shipment::$valid_statuses['landed']);
            })
            ->where(function($query) {
